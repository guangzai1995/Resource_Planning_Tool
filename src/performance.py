    ...
    def calculate_max_concurrency_optimized(self,gpu_model, model_name, card_count, 
        ...
        try:
            ...
            if (not gpu_model or not model_name or not card_count or not input_length or
                ...
                # 统一返回值为 (result, chart, max_concurrency)
                # 返回结构化信息，避免上层解析富文本
                return {
                    ...
                    "input": {
                        ...
                        "max_first_token_delay": max_first_token_delay,
            ...
            if per_user_throughput >= user_throughput:
                ...
                if optimal_concurrency is None:
                    return {
                        "message": f"警告: 无法找到同时满足延时和单用户吞吐量要求(≥{user_throughput} tokens/s)的配置！",
                        "reason": "constraints_unsatisfied",
                    }, None, 0
            ...
            result = {
                ...
                "output_length": output_length,
                "max_first_token_delay": max_first_token_delay,
                "user_throughput_requirement": user_throughput,
                "optimal": {
                    "concurrency": int(optimal_concurrency),
                    "total_throughput": float(optimal_throughput),
                    "per_user_throughput": float(optimal_per_user),
                    "interpolation_used": bool(interpolation_used),
                },
                "boundary": {
                    "max_acceptable_concurrency": float(max_acceptable_concurrency),
                    "estimated_total_throughput": float(estimated_throughput),
                },
            }
            ...
            # 保持三元结构，便于上层统一处理
            return {"message": f"计算出错: {str(e)}", "reason": "calculation_error"}, None, None

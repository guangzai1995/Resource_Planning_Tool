...
def add_dataset_parser(parser: argparse.ArgumentParser) -> None:
    ...
    g.add_argument(
        ...
        help='数据集类型（shim 支持: random / sharegpt / custom_audio；'
             '其他类型请安装完整 vllm 包）',
    )
    g.add_argument(
    ...
    def _decode_to_target_len(token_ids: list[int],
        ...
        for _ in range(max_retry + 1):
            ...
            if actual_len > target_len:
                ...
                current_ids.extend(
                    random.randrange(vocab_size)
                    for _ in range(target_len - actual_len)
                )

        text = tokenizer.decode(current_ids)
        return text, _encode_len(text)
...
def get_samples(args: argparse.Namespace, tokenizer) -> list[SampleRequest]:
    ...
    当前 shim 支持: random / sharegpt / custom_audio。
    """
    ...
    if name in ('random', 'random-mm'):
        ...
        raise NotImplementedError(
            ...
            f"shim 支持的数据集: random / sharegpt / custom_audio\n"

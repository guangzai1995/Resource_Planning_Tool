...
def _extract_row(
    ...
    *,
    ...
    return {
        ...
        'spec_decode_acceptance_length': _f('spec_decode_acceptance_length'),
...
def save_xlsx(rows: List[dict], path: str) -> None:
    ...
    explain = [
        ...
        ('avg_gpu_kv_cache_usage', '平均 GPU KV cache 占用率(%)', 'benchmark 运行期间定期采样 /metrics 中 vLLM GPU KV cache usage 或 SGLang token_usage 后求平均值'),
        ('peak_gpu_kv_cache_usage', '峰值 GPU KV cache 占用率(%)', 'benchmark 运行期间定期采样 /metrics 中 vLLM GPU KV cache usage 或 SGLang token_usage 后取峰值'),
        ('spec_decode_acceptance_rate', 'Spec Decode 接受率(%)', 'vLLM accepted_tokens ÷ draft_tokens × 100；SGLang spec_accept_rate gauge（来自 /metrics）'),
        ('spec_decode_acceptance_length', 'Spec Decode 接受长度', 'vLLM acceptance_length；SGLang spec_accept_length gauge（来自 /metrics）'),
        ('spec_decode_system_efficiency', 'Spec Decode 系统效率', '接受 tokens 相对 draft tokens 的服务端效率指标'),
        ('spec_decode_num_drafts', 'Spec Decode 草稿轮数', 'vLLM num_drafts_total 差分；SGLang spec_verify_calls_total 差分'),
        ('spec_decode_num_accepted_tokens', 'Spec Decode 接受 tokens 数', 'vLLM num_accepted_tokens_total 差分；SGLang 由 draft tokens × accept rate 推导'),
        ('spec_decode_num_draft_tokens', 'Spec Decode 草稿 tokens 数', 'vLLM num_draft_tokens_total 差分；SGLang verify_calls × spec_num_draft_tokens 推导'),

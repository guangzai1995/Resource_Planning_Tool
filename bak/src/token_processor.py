...
class InitTokenizer:
    ...
    def calculate_input_tokens(self,system_prompt, user_prompt):
        ...
        # 生成详细报告
        report = (
            f'📋 <span style="color:{COLOR_PALETTE["标注"]}; font-weight:bold">Token 统计报告</span>\n\n'  # 外层用单引号包裹
             f'<span style="color:{COLOR_PALETTE["趋势线"]}">• 系统提示词</span>: {system_tokens} tokens ({system_chars} 字符)\n\n'
            f'<span style="color:{COLOR_PALETTE["趋势线"]}">• 用户输入</span>: {user_tokens} tokens ({user_chars} 字符)\n\n'
            f'🔺 <span style="color:{COLOR_PALETTE["错误"]}; font-weight:bold">总计</span>: {total_tokens} tokens ({total_chars} 字符)\n\n'
            #f'📈 <span style="color:{COLOR_PALETTE["标注"]}; font-weight:bold">使用建议</span>:\n'
            #f'🎯 推荐输入长度: {min([x for x in [2048, 4096, 8192, 12000, 16000, 20000] if x >= total_tokens], default=20000)}\n'
            #f'📌 最小要求长度: {total_tokens}\n\n'
            #f'<span style="color:#888; font-size:0.9em">💡 提示：实际性能测试时请选择≥推荐值的输入长度</span>'
        )
        
        return report  # 只返回一个值，不是元组

...
def check_multi_step(args, api_url, tokenizer, prompt_len, output_len):
    ...
    # 计算理论 chunk 数量
    # 首 token + 中间 token 对应的 chunk + 结束标记（如果是 openai）
    theory_return_num = 1  # 第一个 token 总是单独返回
    # 剩余 token 的 chunk 数量（考虑调度步长）
    remaining_tokens = output_len - 1
    if remaining_tokens > 0:
        # 计算完整调度步数的 chunk
        theory_return_num += (remaining_tokens - 1) // args.num_scheduler_steps + 1
        # 如果剩余 token 数正好被步长整除，则不需要额外 chunk，否则需要最后一个 chunk 处理剩余 token
        # 注意：这里已经包含了所有 token 的返回，因此无需额外调整

    # 如果是 openai/openai-chat/vllm-chat 后端，最后会有一个 [DONE] 标记
    end_chunk_backends = {"openai", "openai-chat", "vllm-chat"}
    end_chunk_num = 1 if args.backend in end_chunk_backends else 0
    theory_return_num += end_chunk_num

    # 检查实际接收的 chunk 数量是否在理论值范围内（允许合并少一个 chunk）
    # 同时检查最后一个 chunk 是否包含多个数据事件（如合并情况）
    min_expected = theory_return_num - 1  # 允许合并少一个 chunk
    max_expected = theory_return_num
    
    # 检查最后一个 chunk 是否包含多个数据事件
    last_chunk = chunks[-1] if chunks else b''
    if b'\ndata: ' in last_chunk or b'\n\ndata: ' in last_chunk:
        # 如果最后一个 chunk 包含多个数据事件，则实际需要的 chunk 数量可减少
        # 此时允许实际数量比理论值少 1
        min_expected = theory_return_num - 1
    else:
        min_expected = theory_return_num  # 无合并时，要求严格匹配

    print(f"theory_return_num: {theory_return_num}, return_num: {return_num}, allowed range: [{min_expected}, {max_expected}]")
    
    if min_expected <= return_num <= max_expected:
        return True
    return False

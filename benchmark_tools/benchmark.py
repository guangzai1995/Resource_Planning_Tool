...
def main(args: argparse.Namespace):
    ...
    api_key_arg = f" --api-key {args.api_key}" if args.api_key else ""
    subprocess.run(
        ...
        f"--benchmark-csv {os.path.join(output_dir, 'benchmark_serving_alpaca.csv')}" + api_key_arg,
        check=True)
    ...
    subprocess.run(
        ...
        f"--max-tokens 1024 --max-prompt-tokens 900 "
        f"--benchmark-csv {os.path.join(output_dir, 'benchmark_serving_sharegpt.csv')}" + api_key_arg,
        check=True)
    ...
    subprocess.run(
        ...
        f"--output-tokens 256 256  --prompt-tokens 512 1024 --benchmark-csv {benchmark_parallel_csv}" + api_key_arg,
        check=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="API密钥（可选）。不传则不加 Authorization 头。",
    )

...
def main(args: argparse.Namespace):
    ...
    all_latency_record = []
    # 按prompt_tokens分组，对每个prompt_tokens情形遍历不同并发数
    # 如果某并发数不满足约束，则跳过该prompt_tokens的更高并发测试
    # 保证并发数从小到大遍历
    sorted_parallel_nums = sorted(args.parallel_num)
    for i, prompt_tokens in enumerate(args.prompt_tokens):
        output_tokens = args.output_tokens[i]
        skip_higher_parallel = False  # 用于标记是否跳过更高并发测试

        for parallel_num in sorted_parallel_nums:
            if skip_higher_parallel:
                logger.warning(
                    f"跳过 prompt_tokens={prompt_tokens} 并发数={parallel_num} 的测试（已超出约束）"
                )
                continue
            ...
            metrics = statistics_and_print_performance_data(args, prompt_tokens, output_tokens, parallel_num,
                                                  request_latency_record, all_latency_record)
            
            # 检查约束条件
            if metrics is not None:
                avg_first_token_latency = metrics.get('avg_first_token_latency', 0)
                total_throughput = metrics.get('total_throughput', 0)
                per_parallel_throughput = total_throughput / parallel_num if parallel_num > 0 else 0
                
                # 检查平均首token延时约束
                if args.max_avg_first_token_latency is not None and args.max_avg_first_token_latency > 0:
                    if avg_first_token_latency > args.max_avg_first_token_latency:
                        logger.warning(
                            f"约束检查失败: 平均首token延时 {avg_first_token_latency:.2f}ms > 约束值 {args.max_avg_first_token_latency}ms, "
                            f"将跳过 prompt_tokens={prompt_tokens} 的更高并发测试"
                        )
                        skip_higher_parallel = True
                
                # 检查单并发吞吐量约束
                if args.min_per_parallel_throughput is not None and args.min_per_parallel_throughput > 0:
                    if per_parallel_throughput < args.min_per_parallel_throughput:
                        logger.warning(
                            f"约束检查失败: 单并发吞吐量 {per_parallel_throughput:.2f} tokens/s < 约束值 {args.min_per_parallel_throughput} tokens/s, "
                            f"将跳过 prompt_tokens={prompt_tokens} 的更高并发测试"
                        )
                        skip_higher_parallel = True
                ...
                if not skip_higher_parallel:
                    logger.info(
                        f"约束检查通过: 平均首token延时={avg_first_token_latency:.2f}ms, 单并发吞吐量={per_parallel_throughput:.2f} tokens/s"
                    )
...
if __name__ == "__main__":
    ...
    # 新增约束参数
    parser.add_argument("--max-avg-first-token-latency", type=float, default=None,
                        help="平均首token延时约束（毫秒）。如果某并发数的平均首token延时超过此值，则跳过该prompt-tokens的更高并发测试。")
    parser.add_argument("--min-per-parallel-throughput", type=float, default=None,
                        help="单并发吞吐量约束（tokens/s）。计算方式为总吞吐量/并发数。如果低于此值，则跳过该prompt-tokens的更高并发测试。")
    parser.add_argument(
        "--api-key",
        type=str,
        default=(
            os.environ.get("BENCHMARK_API_KEY")

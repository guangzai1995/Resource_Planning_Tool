...
def warm_up(engine, adapters, num_tokens, lora_adapters_path, identical_lora_id, batch_size):
    """Process one warm up request for building graph"""
    run_tests(engine, adapters, num_tokens, lora_adapters_path, identical_lora_id, batch_size)
...
def get_test_prompts(adapters, batch_size, num_tokens, lora_adapters_path, workload_type, identical_lora_id):
    ...
    return [create_test_prompts(index + 1, adapters[index]["prompt"],
                                f'{lora_adapters_path}/{adapters[index]["name"]}',
                                num_tokens) for index in sample]

def run_tests(engine, adapters, num_tokens, lora_adapters_path, identical_lora_id, batch_size, workload_type="distinct"):

    test_prompts = get_test_prompts(adapters, batch_size, num_tokens, lora_adapters_path, workload_type, identical_lora_id)
...
def init():
    ...
    common_config["report_dir"] += "/npunica/"
    Path(common_config["report_dir"]).mkdir(parents=True, exist_ok=True)

    engine = initialize_engine(common_config)
    return engine, adapters, common_config

def main():
    engine, adapters, common_config = init()
    
    latencies = pd.DataFrame(columns=["batch_size", "prefill_latency_ms",
    ...
    for batch_size in batch_sizes:
        ...
        warm_up(engine, adapters, common_config["warmup_num_tokens"], common_config["lora_adapters_path"], common_config["identical_lora_id"], batch_size)

        process_data, per_batch_latency, step_latency_df = run_tests(engine, adapters, common_config["num_tokens"],
        ...
        latencies = latencies.append(per_batch_latency, ignore_index=True)
